  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 p-5">
      <div className="flex items-center justify-between mb-4">
        <label className={`text-xs font-bold uppercase tracking-wider ${mutedText}`}>JWT Token Decoder</label>
      ...
      <textarea
        ...
        className={`w-full resize-none rounded-lg p-4 font-mono text-sm leading-relaxed border focus:outline-none transition-colors duration-150 mb-4 ${isLight ? "bg-white border-gray-200 focus:border-green-400 text-gray-900" : "bg-[#0a0a0a] border-[#1a1a1a] focus:border-[#00e676] text-[#e0e0e0]"}`}
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="flex items-center justify-between mb-2">
              ...
              {result.data.isExpired !== null && (
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded ${result.data.isExpired ? "bg-red-500/10 text-red-400 border border-red-500/20" : "bg-green-500/10 text-green-400 border border-green-500/20"}`}>
          ...
          {(result.data.expiresAt || result.data.issuedAt) && (
            <div className={`rounded-lg border p-3 ${isLight ? "bg-gray-50 border-gray-200" : "bg-[#0d0d0d] border-[#1a1a1a]"}`}>
              ...
              <div className={`flex flex-wrap gap-4 text-xs font-mono ${isLight ? "text-gray-600" : "text-[#888]"}`}>
                {result.data.issuedAt && <div><span className={mutedText}>Issued: </span><span className="text-[#00e676]">{result.data.issuedAt}</span></div>}
                {result.data.expiresAt && <div><span className={mutedText}>Expires: </span><span className="text-[#00e676]">{result.data.expiresAt}</span></div>}
              </div>
            </div>
          )}
        </div>
