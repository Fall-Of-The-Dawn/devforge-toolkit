export default function AdBanner({ config, theme }) {
  ...
  return (
    <div className={`rounded-lg p-4 flex items-start gap-3 border transition-colors duration-200 ${isLight ? "bg-gray-50 border-gray-200" : "bg-[#0d0d0d] border-[#1a1a1a]"}`}>
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${isLight ? "bg-[#fff0f0]" : "bg-[#1a1a1a]"}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <a href={config.ctaUrl} className="text-xs font-bold text-[#FF6B6B] hover:text-[#c53a3a] transition-colors">
          {config.ctaText} →
        </a>
      </div>
    </div>
