...
const SHADE_MAP = { 50:50,100:100,200:200,300:300,400:400,500:500,600:600,700:700,800:800,900:900,950:950 };

function resolveColor(className) {
  // bg-{color}-{shade}, text-{color}-{shade}, border-{color}-{shade}
  const match = className.match(/^(bg|text|border|ring|shadow|outline|accent|caret|decoration|placeholder)-(.+)$/);
  if (!match) return null;
  const prefix = match[1];
  const rest = match[2];

  // Handle named specials
  if (rest === "white") return `${prefix === "bg" ? "background-color" : prefix === "text" ? "color" : "border-color"}: #ffffff;`;
  if (rest === "black") return `${prefix === "bg" ? "background-color" : prefix === "text" ? "color" : "border-color"}: #000000;`;
  if (rest === "transparent") return `${prefix === "bg" ? "background-color" : prefix === "text" ? "color" : "border-color"}: transparent;`;
  if (rest === "current") return `${prefix === "bg" ? "background-color" : prefix === "text" ? "color" : "border-color"}: currentColor;`;

  // Try to find color-shade
  const parts = rest.split("-");
  let colorName, shade;

  // Handle multi-word color names like "dark-blue" -> not valid, but "blue" with "500"
  // Tailwind colors: bg-red-500, bg-slate-200, etc.
  // Try last part as shade first
  const lastPart = parts[parts.length - 1];
  if (SHADE_MAP[lastPart] !== undefined && parts.length >= 2) {
    shade = lastPart;
...
export function resolveTailwindClass(cls) {
  ...
  const colorResult = resolveColor(cls);
